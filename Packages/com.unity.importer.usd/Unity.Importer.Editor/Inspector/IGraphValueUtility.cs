...
namespace UnityEditor.Importer
{
    ...
    {
        // Resolved with nameof so a future API rename breaks compilation instead of failing silently at import time.
        private const string k_LazyLoadAssetProperty = nameof(LazyLoadReference<Object>.asset);
        private const string k_LazyLoadEntityIdProperty = nameof(LazyLoadReference<Object>.entityId);
        ...
        public static List<IGraphValue> ConvertLazyLoadObjectToObject(this IEnumerable<IGraphValue> values, out HashSet<string> overrideIds, out HashSet<EntityId> entityIds)
        {
            ...
            foreach (var value in values)
            {
                ...
                {
                    convertedValues.Add(ConvertToObjectFromLazyLoad(value, entityIds));
                }
                else if (value.Type.IsArray
                         && value.Type.GetElementType().IsGenericType
                         && value.Type.GetElementType().GetGenericTypeDefinition() == typeof(LazyLoadReference<>))
                {
                    convertedValues.Add(ConvertToObjectFromLazyLoadArray(value, entityIds));
                }
                ...
                {
                    convertedValues.Add(ConvertToObjectFromLazyLoadList(value, entityIds));
                }
                else
                {
        ...
        private static IGraphValue ConvertToObjectFromLazyLoad(IGraphValue value, HashSet<EntityId> entityIds)
        {
            var lazyLoadAssetGetter = value.Type.GetProperty(k_LazyLoadAssetProperty);
            var lazyLoadEntityIdGetter = value.Type.GetProperty(k_LazyLoadEntityIdProperty);
            ...
            entityIds.Add((EntityId)lazyLoadEntityIdGetter.GetValue(value.Value));

            return Activator.CreateInstance(importSettingType, value.Id, objectValue) as IGraphValue;
        }

        private static IGraphValue ConvertToObjectFromLazyLoadArray(IGraphValue value, HashSet<EntityId> entityIds)
        {
            var lazyLoadAssetGetter = value.Type.GetElementType().GetProperty(k_LazyLoadAssetProperty);
            var lazyLoadEntityIdGetter = value.Type.GetElementType().GetProperty(k_LazyLoadEntityIdProperty);
            ...
            for (int j = 0; j < objectArray.Length; j++)
            {
                ...
                entityIds.Add((EntityId)lazyLoadEntityIdGetter.GetValue(valueArray.GetValue(j)));
            }
        ...
        private static IGraphValue ConvertToObjectFromLazyLoadList(IGraphValue value, HashSet<EntityId> entityIds)
        {
            // List<LazyLoadReference<T>> has no element type, so read the generic argument instead.
            var lazyLoadType = value.Type.GetGenericArguments()[0];
            var lazyLoadAssetGetter = lazyLoadType.GetProperty(k_LazyLoadAssetProperty);
            var lazyLoadEntityIdGetter = lazyLoadType.GetProperty(k_LazyLoadEntityIdProperty);
            ...
            for (int j = 0; j < valueList.Count; j++)
            {
                ...
                entityIds.Add((EntityId)lazyLoadEntityIdGetter.GetValue(valueList[j]));
            }
        ...
        public static void DependsOnArtifact(this AssetImportContext ctx, EntityId entityId)
        {
            if (entityId.IsValid())
            {
                if (AssetDatabase.TryGetGUIDAndLocalFileIdentifier(entityId, out var guid, out long _))
                {
