...
namespace UnityEditor.Importer
{
    ...
    public abstract class ModularImporter : ScriptedImporter
    {
        ...
        public override void OnImportAsset(AssetImportContext ctx)
        {
            ...
            var overrideSettings = m_ImportSettingOverrides.ConvertLazyLoadObjectToObject(out var overrideIds, out var entityIds);
            RegisterReferencesDependencies(ctx, tempGraph, overrideIds, entityIds);
            ...
            HashSet<EntityId> entityIds)
        {
            ...
            while (prop.Next(true) && prop.propertyPath != end.propertyPath)
            {
                if (prop.propertyType == SerializedPropertyType.ObjectReference)
                {
                    ...
                    // Read the reference's EntityId directly rather than stepping into the PPtr's
                    // int m_FileID child, which can't hold a 64-bit EntityId.
                    entityIds.Add(prop.objectReferenceEntityIdValue);
                }
            }

            foreach (var entityId in entityIds)
            {
                ctx.DependsOnArtifact(entityId);
            }
        }
